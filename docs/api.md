...
`price_date` — строка `YYYY-MM-DD`, независимо от имени файла.
...
`imported_at`, `sha256`. Возраст прайса сайт определяет по `price_date`,
если она задана; `imported_at` не доказывает дату составления файла.
...
запрос/лимит/дата; `OSError` — ошибка чтения источника или создания директории.
